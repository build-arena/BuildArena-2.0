...
def descriptor(self: Block) -> str:
    """Stroke axis along the mount face normal: extend +n, retract −n."""
    ...
    if axis_norm <= 1e-9:
        return "Piston stroke axis is undefined (degenerate normal)."
    axis_unit = axis_virtual / axis_norm
    extend_vec = Vector(vector=axis_unit)
    retract_vec = Vector(vector=-axis_unit)
    return (
        f"Piston stroke axis: extends toward {extend_vec.caption} (outward, positive face normal), "
