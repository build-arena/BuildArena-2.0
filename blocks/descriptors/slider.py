...
def descriptor(self: Block) -> str:
    """Stroke axis along the mount face normal; algorithm matches piston.py (extend +n, retract −n)."""
    ...
    if axis_norm <= 1e-9:
        return "Slider stroke axis is undefined (degenerate normal)."
    axis_unit = axis_virtual / axis_norm
    extend_vec = Vector(vector=axis_unit)
    retract_vec = Vector(vector=-axis_unit)
    return (
        f"Slider stroke axis: extends toward {extend_vec.caption} (outward, positive face normal), "
        f"retracts toward {retract_vec.caption} (negative face normal)."
    )
